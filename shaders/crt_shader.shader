...
uniform float curvature_x_amount : hint_range(3.0, 15.0, 0.01) = float(0); 
uniform float curvature_y_amount : hint_range(3.0, 15.0, 0.01) = float(0);
...
uniform float horizontal_scan_lines_amount : hint_range(0.0, 180.0, 0.1) = 1080.0;
uniform float horizontal_scan_lines_opacity : hint_range(0.0, 1.0, 0.01) = 0.1;
uniform bool show_vertical_scan_lines = false;
uniform float vertical_scan_lines_amount : hint_range(0.0, 500.0, 0.1) = 500.0;
uniform float vertical_scan_lines_opacity : hint_range(0.0, 1.0, 0.01) = 1.0;
uniform float boost : hint_range(1.0, 2.0, 0.01) = 1.2;
uniform float aberration_amount : hint_range(0.0, 10.0, 0.01) = 1.9;
